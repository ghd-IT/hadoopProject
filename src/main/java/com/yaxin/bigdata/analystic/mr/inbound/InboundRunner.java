...
public class InboundRunner implements Tool {
    private static final Logger logger = Logger.getLogger(InboundRunner.class);
    private Configuration conf = new Configuration();

    public static void main(String[] args) {
        try {
            ToolRunner.run(new Configuration(), new InboundRunner(), args);
        } catch (Exception e) {
...
//        conf.addResource("total_mapping.xml");//修改1
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJarByClass(InboundRunner.class);
        job.setMapperClass(InboundMapper.class);
        job.setMapOutputKeyClass(StatsInboundDimension.class);
        job.setMapOutputValueClass(InboundOutputValue.class);
        job.setReducerClass(InboundReducer.class);
        job.setOutputKeyClass(StatsInboundDimension.class);
