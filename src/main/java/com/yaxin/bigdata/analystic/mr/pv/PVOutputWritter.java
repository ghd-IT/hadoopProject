 ...
 * @since 1.0.0
 */
public class PVOutputWritter implements IOutputWritter {
    private static final Logger logger = Logger.getLogger(PVOutputWritter.class);
    @Override
    ...
    public void output(Configuration conf, StatsBaseDimension key, StatsOutputValue value, PreparedStatement ps, IDimension iDimension) {
        ...
        try {
            ...
            //获取新增用户的值
            int url = ((IntWritable)(v.getValue().get(new IntWritable(-1)))).get();
